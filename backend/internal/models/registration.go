...
func (f *RegistrationForm) Sanitize() {
	if f.JenisDaftar != nil    { *f.JenisDaftar = strings.TrimSpace(*f.JenisDaftar) }
	if f.Gender != nil         { *f.Gender = strings.TrimSpace(*f.Gender) }
	if f.Citizenship != nil    { *f.Citizenship = strings.TrimSpace(*f.Citizenship) }
	if f.BirthPlace != nil     { *f.BirthPlace = html.EscapeString(strings.TrimSpace(*f.BirthPlace)) }
	if f.BirthDate != nil      { *f.BirthDate = strings.TrimSpace(*f.BirthDate) }
	if f.PhoneNumber != nil    { *f.PhoneNumber = strings.TrimSpace(*f.PhoneNumber) }
	if f.WhatsappNumber != nil { *f.WhatsappNumber = strings.TrimSpace(*f.WhatsappNumber) }

	if f.PreviousUniversity != nil { *f.PreviousUniversity = html.EscapeString(strings.TrimSpace(*f.PreviousUniversity)) }
	if f.PreviousMajor != nil      { *f.PreviousMajor = html.EscapeString(strings.TrimSpace(*f.PreviousMajor)) }
	if f.Gpa != nil                { *f.Gpa = strings.TrimSpace(*f.Gpa) }
	if f.HighestEducation != nil   { *f.HighestEducation = strings.TrimSpace(*f.HighestEducation) }
	if f.SchoolOrigin != nil       { *f.SchoolOrigin = html.EscapeString(strings.TrimSpace(*f.SchoolOrigin)) }
	
	if f.MajorChoice != nil        { *f.MajorChoice = strings.TrimSpace(*f.MajorChoice) }
	if f.WaktuKuliah != nil        { *f.WaktuKuliah = strings.TrimSpace(*f.WaktuKuliah) }
	if f.HighschoolGpa != nil      { *f.HighschoolGpa = strings.TrimSpace(*f.HighschoolGpa) }
	if f.HighschoolGraduateYear != nil { *f.HighschoolGraduateYear = strings.TrimSpace(*f.HighschoolGraduateYear) }

	if f.ContactEmail != nil { *f.ContactEmail = strings.ToLower(strings.TrimSpace(*f.ContactEmail)) }
	if f.Religion != nil     { *f.Religion = strings.TrimSpace(*f.Religion) }
	if f.FundingSource != nil { *f.FundingSource = strings.TrimSpace(*f.FundingSource) }
	if f.Address != nil      { *f.Address = html.EscapeString(strings.TrimSpace(*f.Address)) }
	if f.SubDistrict != nil  { *f.SubDistrict = html.EscapeString(strings.TrimSpace(*f.SubDistrict)) }
	if f.District != nil     { *f.District = html.EscapeString(strings.TrimSpace(*f.District)) }
	if f.Hamlet != nil       { *f.Hamlet = html.EscapeString(strings.TrimSpace(*f.Hamlet)) }
	if f.PostalCode != nil   { *f.PostalCode = strings.TrimSpace(*f.PostalCode) }
	if f.Rt != nil           { *f.Rt = strings.TrimSpace(*f.Rt) }
	if f.Rw != nil           { *f.Rw = strings.TrimSpace(*f.Rw) }

	if f.TaxID != nil            { *f.TaxID = strings.TrimSpace(*f.TaxID) }
	if f.Reference != nil        { *f.Reference = html.EscapeString(strings.TrimSpace(*f.Reference)) }
	if f.ExpertField != nil      { *f.ExpertField = html.EscapeString(strings.TrimSpace(*f.ExpertField)) }
	if f.Degree != nil           { *f.Degree = html.EscapeString(strings.TrimSpace(*f.Degree)) }
	if f.CompanyName != nil      { *f.CompanyName = html.EscapeString(strings.TrimSpace(*f.CompanyName)) }
	if f.CompanyAddress != nil   { *f.CompanyAddress = html.EscapeString(strings.TrimSpace(*f.CompanyAddress)) }
	if f.Position != nil         { *f.Position = html.EscapeString(strings.TrimSpace(*f.Position)) }
	if f.CompanyStatus != nil    { *f.CompanyStatus = strings.TrimSpace(*f.CompanyStatus) }
	if f.CompanyStartYear != nil { *f.CompanyStartYear = strings.TrimSpace(*f.CompanyStartYear) }

	if f.FatherName != nil       { *f.FatherName = html.EscapeString(strings.TrimSpace(*f.FatherName)) }
	if f.FatherPhone != nil      { *f.FatherPhone = strings.TrimSpace(*f.FatherPhone) }
	if f.FatherNik != nil        { *f.FatherNik = strings.TrimSpace(*f.FatherNik) }
	if f.FatherBirthdate != nil  { *f.FatherBirthdate = strings.TrimSpace(*f.FatherBirthdate) }
	if f.FatherEducation != nil  { *f.FatherEducation = strings.TrimSpace(*f.FatherEducation) }
	if f.FatherOccupation != nil { *f.FatherOccupation = strings.TrimSpace(*f.FatherOccupation) }
	if f.FatherIncome != nil     { *f.FatherIncome = strings.TrimSpace(*f.FatherIncome) }
	if f.FatherStatus != nil     { *f.FatherStatus = strings.TrimSpace(*f.FatherStatus) }

	if f.MotherName != nil       { *f.MotherName = html.EscapeString(strings.TrimSpace(*f.MotherName)) }
	if f.MotherPhone != nil      { *f.MotherPhone = strings.TrimSpace(*f.MotherPhone) }
	if f.MotherNik != nil        { *f.MotherNik = strings.TrimSpace(*f.MotherNik) }
	if f.MotherBirthdate != nil  { *f.MotherBirthdate = strings.TrimSpace(*f.MotherBirthdate) }
	if f.MotherEducation != nil  { *f.MotherEducation = strings.TrimSpace(*f.MotherEducation) }
	if f.MotherOccupation != nil { *f.MotherOccupation = strings.TrimSpace(*f.MotherOccupation) }
	if f.MotherIncome != nil     { *f.MotherIncome = strings.TrimSpace(*f.MotherIncome) }
	if f.MotherStatus != nil     { *f.MotherStatus = strings.TrimSpace(*f.MotherStatus) }

	if f.ParentsAddress != nil   { *f.ParentsAddress = html.EscapeString(strings.TrimSpace(*f.ParentsAddress)) }

	if f.AccountHolder != nil    { *f.AccountHolder = html.EscapeString(strings.TrimSpace(*f.AccountHolder)) }
	if f.Bank != nil             { *f.Bank = html.EscapeString(strings.TrimSpace(*f.Bank)) }
}
...
type RegistrationForm struct {
	...
	Confirmation *bool `json:"confirmation" validate:"omitempty"`
	...
	Pernyataan *bool `json:"pernyataan" validate:"omitempty"`
