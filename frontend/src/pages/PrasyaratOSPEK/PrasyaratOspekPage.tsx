        ...
        <div className={styles.uploadGroup}>
            <div className={styles.fileInputWrapper}>
                ...
                <input ref={inputRef} type="file" accept=".jpg,.jpeg,.png,.pdf"
                    onChange={(e: ChangeEvent<HTMLInputElement>) => onChange(field.name, e.target.files?.[0] ?? null)} />
            </div>
            <div className={styles.uploadLinks}>
                {field.contohUrl && (
                    <p>Contoh {field.label} :{" "}
                        <a href={field.contohUrl} target="_blank" rel="noopener noreferrer">Klik Untuk Download</a>
                    </p>
                )}
                {field.uploadedUrl && (
                    <p>{field.label} Terupload :{" "}
                        <a href={field.uploadedUrl} target="_blank" rel="noopener noreferrer">Klik Untuk Download (Download)</a>
                    </p>
                )}
            </div>
        </div>
...
const UPLOAD_FIELDS: UploadField[] = [
    { name: "pasFoto", label: "Pas Photo Final (Untuk KTM)", contohUrl: "/files/contoh-pas-foto.jpg", uploadedUrl: undefined },
    { name: "ijazah", label: "Ijazah", contohUrl: undefined, uploadedUrl: undefined },
];
        ...
        <main className={styles.page}>
            <div className={styles.container}>
                ...
                {UPLOAD_FIELDS.map(field => (
                    <FileUploadRow key={field.name} field={field}
                        file={files[field.name]} onChange={handleFileChange} />
                ))}

                <div className={styles.bottomActions}>
                    <button className={styles.btnDanger} onClick={() => navigate(-1)}>Kembali</button>
                    <button className={styles.btn} onClick={handleUpload} disabled={isLoading}>
                        {isLoading ? <><div className={styles.spinner} aria-hidden="true" /> Uploading...</> : "Upload"}
                    </button>
                </div>
            </div>
        </main>
