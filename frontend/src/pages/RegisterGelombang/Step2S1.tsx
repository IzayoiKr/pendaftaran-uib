...
export default function Step2S1({ formData, setFormData, prev, submit, goToStep, currentStep }: Props) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileList?.[0]) {
      ...
      // 🔥 PERBAIKAN: Langsung simpan di formData termasuk nama filenya agar tidak hilang saat tekan back
      setFormData({
        ...formData,
        [name]: file,
        [`${name}Name`]: file.name,
  ...
  // Helper render diperbarui membaca langsung dari formData
        ...
        <p className={styles.uploadedDoc}>
          ...
          {formData[name] ? (
            <>
              ...
              {" - "}
              <a href={formData[`${name}Url`]} target="_blank" rel="noreferrer">
                Lihat/Download
              </a>
            </>
          ) : (
      ...
      <div className={styles.formWrapper}>
        ...
        <div className={styles.statusSection}>
          ...
          <div className={styles.infoBox}>
            ...
            <div className={styles.statusItem}><strong>Status Pemeriksaan Pembayaran:</strong><span className={styles.statusValue}>{paymentStatus}</span></div>
          </div>
        ...
        <div className={styles.documentsSection}>
          ...
          {renderFileInput("Dokumen Kartu Keluarga", "kk")}
        </div>
        ...
        <div className={styles.paymentSection}>
          ...
          <div className={styles.infoBox}>
            <div className={styles.paymentInfo}>
              <p><strong>Nama Bank :</strong> OCBC NISP</p>
              <p><strong>No Rekening :</strong> 094800007802</p>
              <p><strong>Nama Pemilik Rekening :</strong> Universitas Internasional Batam</p>
              <p><strong>Biaya Formulir: </strong> Rp 250.000</p>
            </div>
          </div>
        ...
        <div className={form.buttonGroup}>
          <button type="button" onClick={prev} className={`${form.btn} ${form.btnDanger}`}>Batal (Kembali)</button>
          <button type="button" onClick={handleSubmit} className={`${form.btn} ${form.btnPrimary}`}>Upload (Lanjut)</button>
        </div>
      </div>
    </div>
  );
