...
export async function downloadStaticPdf(
    ...
    const res = await fetchProtected(`/files/${encodeURIComponent(key)}`);
...
export async function viewProtectedPdf(
    endpoint: string,
    filename: string,
): Promise<void> {
    const res = await fetchProtected(endpoint);
    ...
    if (!res.ok) {
        const text = await res.text().catch(() => "");
        throw new Error(text || `Gagal memuat file: ${filename}`);
    }
    ...
    // Clean up blob URL when the tab is closed?
    // Hard to detect tab close, but modern browsers handle this okay.
    // Or just let it be.
}
