...
function mapKelas(v: string): KelasKuliah {
    return (v ?? "").toLowerCase().includes("pagi") ? "Kelas Pagi" : DEFAULT_KELAS;
}
...
function mapBeasiswa(batchName: string): JenisBeasiswa {
    ...
    if (b.includes("insan") || b.includes("mandiri"))        return "Beasiswa Insan Mandiri";
    if (b.includes("beasiswa") || b.includes("i") || b.includes("1")) return "Beasiswa I";
