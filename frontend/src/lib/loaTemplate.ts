...
const htmlTemplate = fs.readFileSync(
    path.join(LIB_DIR, "loaTemplate.html"),
    "utf-8",
);
const scssSource = fs.readFileSync(
    path.join(LIB_DIR, "loaTemplate.scss"),
    "utf-8",
);
...
function buildBiayaRows(data: LoaData): string {
    ...
    return [
        ...
        biaya.bppPraktikum > 0
            ? row(
                  "&nbsp;&nbsp;3) BPP Praktikum",
                  formatRupiah(biaya.bppPraktikum),
              )
            : "",
        ...
        calc.potonganBppPokok > 0
            ? row(
                  `&nbsp;&nbsp;Potongan BPP Pokok (${potongan.potonganBppPokok}%)`,
                  formatRupiah(calc.potonganBppPokok),
              )
            : "",
        calc.potonganBppSks > 0
            ? row(
                  `&nbsp;&nbsp;Potongan BPP SKS (${potongan.potonganBppSks}%)`,
                  formatRupiah(calc.potonganBppSks),
              )
            : "",
        calc.potonganBppPraktikum > 0
            ? row(
                  `&nbsp;&nbsp;Potongan BPP Praktikum (${potongan.potonganBppPraktikum}%)`,
                  formatRupiah(calc.potonganBppPraktikum),
              )
            : "",
        calc.potonganSpp > 0
            ? row(
                  `&nbsp;&nbsp;Potongan SPP (${potongan.potonganSpp}%)`,
                  formatRupiah(calc.potonganSpp),
              )
            : "",
    ].join("\n");
}
